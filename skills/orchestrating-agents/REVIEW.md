...
- All review panes **must open in the same tmux session** as the Orchestrating Agent.
- `open-review-pane.sh` resolves the current session via `$TMUX` and `tmux display-message -p '#S'`.
- A new window is created within that session — **never a new tmux session**.
...
2. Call `open-review-pane.sh "review-<task-id>" "<worktree-path>"` — opens a window showing `git diff HEAD`.
3. Present the full diff to the human and ask for approval.
4. **On approval:**
   a. Call `close-review-pane.sh "review-<task-id>"`.
...
5. Once the Task Agent has implemented and pushed the approved change:
   a. Call `open-review-pane.sh "review-update-<task-id>" "<worktree-path>"`.
   b. Present the updated diff to the human for confirmation.
   c. Call `close-review-pane.sh "review-update-<task-id>"` after human confirms.
...
3. When the Task Agent reports resolution:
   a. Call `open-review-pane.sh "review-conflict-<task-id>" "<worktree-path>"`.
   b. Present the resolved diff to the human for approval.
   c. **On approval:** call `close-review-pane.sh "review-conflict-<task-id>"`, notify Task Agent to push.
